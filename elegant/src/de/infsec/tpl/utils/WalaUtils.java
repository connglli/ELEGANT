...
public class WalaUtils {
	...
	public static IMethod getIMethod(IClassHierarchy cha, String signature) {  // TODO: throw exceptions
		...
		try {
			...
			System.out.println("Classname " + clazzName + " could not be looked up!");
		}
	...
	public static CGNode getCGNode(IMethod method, CallGraph cg) {
		...
		if (cgnode.isEmpty()) {
			System.err.println("Number of CGNode(s) for " + method.getSignature() + " is " + cgnode.size());
			return null;
		}
		/*else if (cgnode.size() > 1) {
			System.err.println("Number of CGNode(s) for " + methodSignature + " is " + cgnode.size() +  "  refMethod.sig: " + refMethod.getSignature());
		}*/
	...
	public static SSACFG getSSACFG(String methodSignature, CallGraph cg) {
		...
		if (ir == null && !methodSignature.startsWith(WALA_FAKE_ROOT_CLASS))
			System.err.println("Could not retrieve IR for " + methodSignature);
	...
	public static SSAInstruction getSSAInstruction(CallGraph cg, String methodSignature, int iindex) {
		...
		if (cfg == null) {
			System.err.println("getSSAInstruction:: Did not find SSACFG for " + methodSignature);
		} else {
			...
			if (block != null) {
				...
				System.err.println("getSSAInstruction:: Did not find iindex " + iindex + " in SSACFG (" + methodSignature + ")");
			} else
				System.err.println("getSSAInstruction:: Did not find basic block for iindex " + iindex + " in SSACFG (" + methodSignature + ")");
		}
		return null;
	}
